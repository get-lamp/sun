...
module.exports = function(measures){
	...
	this.drawGrid = function(view, x,y){
		...
		for(a in angles){
			anglesY.push(dec.degreeArcRadianLength(angles[a], this.radius()) * view.factor());
		}			

		// draw degree lines
		for( let h in anglesY ){
			view.ctx().beginPath();
			view.ctx().moveTo(x, y + anglesY[h]);
		    view.ctx().lineTo(x + w, y + anglesY[h]);
		    view.ctx().stroke();

			view.ctx().fillText( angles[h].toString(), x-10, y + anglesY[h] );
		...
		const holeY = dec.degreeArcRadianLength(360-45, this.radius()) * view.factor();
	...
	this.drawDots = function(view, x, y){
		...
		var o = dec.degreeArcRadianLength(45, this.radius()) * view.factor();
		...
		view.ctx().beginPath();
		view.ctx().rect(x-5, o, w+20, dec.degreeArcRadianLength(225, this.radius()) * view.factor());
		view.ctx().save()
		view.ctx().clip()
		...
		for(s in hours){
			...
			for(i=0; i < YEAR.length; i++){
				...
				h = y + o + (dec.degreeArcRadianLength(alt * 2, this.radius()) * view.factor());
	...
	this.drawCurveThroughNPoints = function(view, points){
		...
		for (var i = 1; i < points.length - 1; i ++) {
			...
			// curve
			//var cpx = (points[i].x + (points[i+1].x - points[i].x) / 2);
			//var cpy = (points[i].y + (points[i+1].y - points[i].y) / 2);
